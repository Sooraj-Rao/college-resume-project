...
import User from "../models/User.js"; // Keep User import for populate
...
router.get("/public/:identifier", async (req, res) => {
  try {
    ...
    let campaignRef = "";
    let referrerSource = "direct"; // Default source
    if (referrerHeader) {
      try {
        const url = new URL(referrerHeader);
        campaignRef = url.searchParams.get("ref") || "";
        referrerSource = url.hostname; // Use hostname as source
      } catch (e) {
        // Ignore invalid URL
      }
    }
    ...
    res.json({
      ...
      resume,
      sessionId,
      // Removed requireReferrer from response
    });
  } catch (error) {
    res.status(500).json({ success: false, message: "Server error" });
  }
